 ...
 *
 */
export class BotRoot<T = { BOT_TOKEN: string; [key: string]: string }> {
  ...
  constructor(
    ...
    if (config.roomId) {
      this.meta.roomId = config.roomId
    }
  ...
  public async locationAuthorizer(
    ...
    const { personId } = trigger.message
    const { id } = await this.send(rootMessage)
    const url = `${this.meta.url}location?roomId=${trigger.message.roomId}&messageId=${id}&personId=${personId}`
    this.send(
      this.warningCard({
        title: message
          ? message
